...
void AWorksheetCCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	//Maps the E button to the change weapon function
	...
	{
		PlayerInputComponent->BindAction("Fire", IE_Pressed, this, &AWorksheetCCharacter::FireType);
	}
...
void AWorksheetCCharacter::Raycast()
{
	...
	//Set the ray to start from the Camera
	FVector StartTrace = FirstPersonCameraComponent->GetComponentLocation();
	//Sets the forward vector to be the direction the player is facing
	...
	if (GetWorld()->LineTraceSingleByChannel(*HitResult, StartTrace, EndTrace, ECC_Visibility, *TraceParams))
	{
		...
		{
			AEnemyClass* CubeEnemy = Cast<AEnemyClass>((*HitResult).GetActor());
			if (CubeEnemy)
			{
				//GEngine->AddOnScreenDebugMessage(-1, 10.0, FColor::Blue, FString::FString("You hit an enemy!"));
				CubeEnemy->Health -= 10.0;
			}
		}
	}
}
...
void AWorksheetCCharacter::FireType()
{
	if (ShouldRaycast)
	{
		Raycast();
