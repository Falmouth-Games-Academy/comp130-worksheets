...
AWorksheetCCharacter::AWorksheetCCharacter()
{
	...
	// RaycastDamage is 51 so as two shots will kill an enemy with health 100
	RaycastDamage = 51;
}
...
void AWorksheetCCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Raycast", IE_Pressed, this, &AWorksheetCCharacter::PerformRaycast);
	PlayerInputComponent->BindAction("GrenadeLauncher", IE_Pressed, this, &AWorksheetCCharacter::FireInTheHole);
	...
	{
		PlayerInputComponent->BindAction("Fire", IE_Pressed, this, &AWorksheetCCharacter::PerformRaycast);
	}
...
void AWorksheetCCharacter::PerformRaycast()
{
	...
	if (GetWorld()->LineTraceMultiByChannel(HitResults, StartTrace, EndTrace, ECC_Visibility, *TraceParams))
	{
		// Uncomment following line to draw a line in the world which mirrors the raycast
		//DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor(255, 0, 0), true);

		for (auto Actors = HitResults.CreateIterator(); Actors; Actors++)
		{
			// If HitResult is an Enemy instance then reduce health
			AMyEnemy* Enemy = Cast<AMyEnemy>((*Actors).GetActor());

			if (Enemy)
			{
				Enemy->Health -= RaycastDamage;
			}
		}
	}
}

void AWorksheetCCharacter::FireInTheHole()
{
	// try and fire a projectile
	if (ProjectileClass != NULL)
	{
